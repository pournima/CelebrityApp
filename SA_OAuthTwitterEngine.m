...
@synthesize consumerSecret = _consumerSecret, consumerKey = _consumerKey,appStatus;
...
- (void) setAccessToken: (OAServiceTicket *) ticket withData: (NSData *) data {
    ...
    NSLog(@"*********** Access Token - %@",_accessToken.key);
    NSLog(@"*********** Access Secret - %@",_accessToken.secret);
    
    [[NSUserDefaults standardUserDefaults] setValue:_accessToken.key forKey:@"TWAUTHKEY"];
    [[NSUserDefaults standardUserDefaults] setValue:_accessToken.secret forKey:@"TWAUTHSECRET"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    /*appStatus = [[AppStatus alloc]init];
    [appStatus saveShareStatus:@"TW_API_KEY" withArg2:_accessToken.key];   
    [appStatus saveShareStatus:@"TW_API_SECRET" withArg2:_accessToken.secret];*/
}
	...
	for (NSString *tuple in tuples) {
		...
		if (keyValueArray.count == 2) {
			...
			if ([key isEqualToString:@"user_id"]) 
            {
                ...
                [[NSUserDefaults standardUserDefaults] synchronize];
                NSLog(@"Twitter USERID %@",[[NSUserDefaults standardUserDefaults] valueForKey:@"USERID"]);
            }
